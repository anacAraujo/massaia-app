...
const ArtPiecesTable = ({ artPieces }) => {
    ...
    const artPiecesData = artPieces.map(artPiece => {
        ...
        return {
            ...
            songName: artPiece.song,
            authorId: artPiece.author_id,
            authorName: artPiece.author,
            image: artPiece.image,
            date: date,
            button: (
                <div>
                    <CTooltip content="Editar" placement="bottom">
                        <CButton onClick={() => OpenEditModal(artPiece.id)} variant='outline' color='warning'>
                            <CIcon icon={cilPencil}></CIcon>
                        </CButton>
                    </CTooltip>
                    <CTooltip content="Apagar" placement="bottom">
                        <CButton onClick={() => OpenDeleteModal(artPiece.id)} className='tableButtons' variant='outline' color='danger'>
    ...
    return (
        <div>
            <div style={{ maxHeight: rows.length > 5 ? '30rem' : 'auto', overflowY: rows.length > 5 ? 'scroll' : 'auto' }}>
                <CTable id={idTable} {...getTableProps} align='middle' responsive captionTop='Tabelas das Obras'>
                    <CTableHead>
                        {HeaderGroups}
                    </CTableHead>
