...
const EditSongs = ({ songId, songData }) => {
    const [name, setName] = useState(songData.map(item => item.name));
    console.log(name);
    const [album_id, setAlbum_id] = useState(songData.map(item => item.album_id));
    console.log(album_id);
    const [albumName, setAlbumName] = useState(songData.map(item => item.album_name));
    console.log(albumName);
    const [position, setPosition] = useState(songData.map(item => item.position));
    console.log(position);
    const [lyrics, setLyrics] = useState(songData.map(item => item.lyrics || null));
    console.log(lyrics);
    const [audio, setAudio] = useState(songData.map(item => item.audio || null));
    console.log(audio);
    const [video, setVideo] = useState(songData.map(item => item.video || null));
    console.log(video);
    const [image, setImage] = useState(songData.map(item => item.image || null));
    console.log(image);
    const [imageName, setImageName] = useState(songData.map(item => item.image ? item.image.split('/').pop() : 'Nenhum ficheiro selecionado.'));
    const [date, setDate] = useState(songData.map(item => item.date || null));
    console.log(date);
    ...
    useEffect(() => {
        setName(songData.map(item => item.name));
        setAlbum_id(songData.map(item => item.album_id));
        setAlbumName(songData.map(item => item.album_name));
        setPosition(songData.map(item => item.position));
        setLyrics(songData.map(item => item.lyrics || null));
        setAudio(songData.map(item => item.audio || null));
        setVideo(songData.map(item => item.video || null));
        setImage(songData.map(item => item.image || null));
        setImageName(songData.map(item => item.image ? item.image.split('/').pop() : 'Nenhum ficheiro selecionado.'));
        setDate(songData.map(item => item.date || null));
    }, [songData]);
